...
export const ROLE_INTENTS = {
  ...
  'card': { use: 'Raised surfaces that sit in the page: cards, inline banners, a chat window, kanban tasks, an empty-state panel.', never: 'Floating layers such as menus and dialogs (those are `popover`), and never the page ground.', pairs: 'card-foreground' },
  ...
  'border': { use: 'Hairlines: outlines of panels and lists, row rules, separators, step connectors (`bg-border` draws a 1px line), chart grid lines.', never: 'The edge of a form control. At 12% it fails the 3:1 non-text rule; controls use `input`.' },
  'input': { use: 'The boundary of anything you type into or toggle (input, select, checkbox, radio, dropzone) and the empty track of a switch, slider or progress bar.', never: 'Decorative dividers or card outlines (too heavy; use `border`), and never text.' },
...
export const TINT_INTENTS = {
  ...
  'tint/foreground/10': { code: '`ring-1 ring-foreground/10`', use: 'The hairline ring around a Card and every floating panel.', never: 'A fill or text; nor dividers inside a surface. Those are `border`.' },
