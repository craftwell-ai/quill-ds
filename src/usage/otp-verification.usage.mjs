export const usage = {
  name: 'otp-verification',
  kind: 'pattern',
  summary: 'A six-digit one-time-passcode entry card for verifying an email, with expiry messaging and a resend action.',
  useWhen: [
    'You need to verify an email with a six-digit one-time passcode and resend.',
  ],
  alternatives: [
    { name: 'login-minimal', when: "you're sending the initial one-time code, not verifying a code the user already received — that's a separate step upstream of this card." },
    { name: 'forgot-password', when: 'recovery works by emailed link, not a typed code.' },
  ],
  rules: [
    {
      id: 'communicate-expiry',
      do: 'State the code\'s expiry window in the card body ("expires in 10 minutes"), not just in the email.',
      dont: 'Leave users guessing why a code that worked a moment ago now fails.',
      visual: true,
    },
  ],
  a11y: [
    'The OTP field\'s accessible name comes from a visually-hidden <Label> (sr-only), since the six boxed slots have no room for visible text.',
    'The six slots read to assistive tech as one field via the shared label, not as six separate unlabeled boxes.',
  ],
  tokens: ['--input', '--ring', '--destructive', '--foreground'],
}
