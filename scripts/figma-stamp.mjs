...
//
//   node scripts/figma-stamp.mjs --block hero navbar   # after /figma-push or a rebuild
...
export function stamp({ blocks = [], tokens = false, allBlocks = false } = {}) {
  ...
  const wanted = new Set(allBlocks ? state.patterns.filter((p) => p.status === 'mirrored').map((p) => p.block) : blocks)
  const stamped = []
