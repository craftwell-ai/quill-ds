...
//
// Emits three artifacts from @material-symbols/svg-200 (outlined):
...
//   3. src/components/ui/icons.generated.d.ts  — `IconName` union over core ∪ lazy (committed).
//
...
export function build() {
  ...
  // Full library reachable by name = core ∪ lazy top-N. Per-icon modules are emitted
  // for the whole set (core included) so any IconName resolves at runtime.
  ...
  // 3) full IconName union
  const nameUnion = full.map((n) => `'${n}'`).join(' | ')
  const coreUnion = core.map((n) => `'${n}'`).join(' | ')
  const dts =
    `// GENERATED by scripts/build-icons.mjs — do not edit.\n` +
    `// Full library reachable by <Icon name> (core sync + lazy per-icon modules).\n` +
  ...
  return { core, lazy: full, coreMjs, dts }
}

if (import.meta.url === `file://${process.argv[1]}`) {
  const { core, lazy } = build()
  console.log(`icons.core.mjs: ${core.length} core | icons/: ${lazy.length} per-icon modules`)
}
