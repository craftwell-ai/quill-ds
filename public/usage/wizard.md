...
### Reach for instead
- **onboarding** — when the steps are a checklist of tasks to complete over time, not a linear form flow completed in one sitting.
- **settings** — when there's only one step and no sequence to track — this is a single settings page, not a flow.

### Rules
- **Do:** Show every step in the stepper up front (done/current/todo), so users know how much is left. **Don't:** Reveal steps one at a time with no sense of total progress.
