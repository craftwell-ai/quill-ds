...
### Reach for instead
- **item** — when the avatar is one part of a larger list row with a title and actions — compose it inside ItemMedia rather than styling Avatar standalone.

### Rules
- **Do:** Always render an AvatarFallback with 1–2 character initials as a sibling of AvatarImage. **Don't:** Render AvatarImage with no AvatarFallback — a broken, slow, or missing image leaves a blank circle with nothing to look at.
- **Do:** Use AvatarGroup to overlap a cluster of avatars and AvatarGroupCount (e.g. "+4") to cap how many render. **Don't:** Lay out many avatars edge-to-edge in a plain flex row — with no overlap or ring they read as an unrelated list, not one group.
- **Do:** Pair AvatarBadge with a visible text label elsewhere in the row (e.g. "Online" in an Item description) when the status it shows must reach screen reader users. **Don't:** Rely on AvatarBadge alone to convey presence or status — it is a decorative, unlabeled span with nothing for assistive tech to announce.
...
### Design tokens
`--paper-deep` · `--ink-muted` · `--line-soft` · `--primary` · `--primary-foreground` · `--background`
